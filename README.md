...
| 기능 | 설명 | 협업 가치 (Human-AI Interaction) |
| :--- | :--- | :--- |
| **🤖 멀티 모델 채팅** | 3종의 LLM(GPT, Gemini, Claude)과 동시 대화 | 단일 모델 편향 방지 및 답변의 다양성 확보 |
| **⚖️ 실시간 비교 모드** | 동일 질문에 대한 3개 모델의 답변을 한 화면에서 비교 | 교차 검증을 통한 비판적 사고(Critical Thinking) 함양 |
| **💡 스마트 모델 추천** | 질문 의도(Skill Tag) 분석 후 최적 모델 자동 추천 | 사용자의 도구 선택 비용 절감 및 학습 효율 증대 |
| **🛤️ 맞춤형 학습 경로** | 과제 유형(리포트, 발표, 코딩)에 따라 단계별 최적 모델을 가이드 | AI 의존도를 낮추고 학습의 전 과정을 주도하는 **스캐폴딩(Scaffolding)** 제공 |
| **🛡️ 환각 경고 시스템** | Fact Error 가능성이 높은 질문 시 주의 메시지 노출 | AI 오류로 인한 학습 피해를 막는 안전장치(Safety Net) |
...
```mermaid
graph LR
    A[Step 1. 아이디어 발산<br>Ideation] -->|Claude 4.5| B[Step 2. 최신 사례 조사<br>Research]
    B -->|Gemini 2.0| C[Step 3. 개요 및 초안<br>Drafting]
    C -->|GPT-4o| D[Step 4. 최종 팩트 체크<br>Fact Check]
    D -->|Human| E[완성<br>Final Output]
    
    style A fill:#f9f,stroke:#333,stroke-width:2px
    style B fill:#bbf,stroke:#333,stroke-width:2px
    style C fill:#bfb,stroke:#333,stroke-width:2px
    style D fill:#fff,stroke:#333,stroke-width:4px,stroke-dasharray: 5 5
